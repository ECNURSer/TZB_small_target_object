...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.device = device
        self.focal_gamma = float(getattr(h, "focal_gamma", 0.0))
    ...
    def classification_loss(
        self, pred_scores: torch.Tensor, target_scores: torch.Tensor, target_scores_sum: torch.Tensor | int
    ) -> torch.Tensor:
        """Return class-balanced BCE, optionally focused on hard examples."""
        targets = target_scores.to(pred_scores.dtype)
        loss = self.bce(pred_scores, targets)
        if self.focal_gamma > 0:
            probabilities = pred_scores.sigmoid()
            p_t = targets * probabilities + (1 - targets) * (1 - probabilities)
            alpha_factor = targets * self.focal_alpha + (1 - targets) * (1 - self.focal_alpha)
            loss *= alpha_factor * (1 - p_t).pow(self.focal_gamma)
        if self.class_weights is not None:
